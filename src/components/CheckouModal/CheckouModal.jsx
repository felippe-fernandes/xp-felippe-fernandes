...
import { faCheck, faExclamation, faRotateLeft, faXmarkSquare } from '@fortawesome/free-solid-svg-icons';
...
function CheckouModal() {
    ...
    useEffect(() => {
        ...
        enableConfirmButton();
    }, [balance, buyQty, sellQty, shareSelected.price, shareSelected.qty, shareSelected.qtyAvailable]);
    ...
    const tableScreen =
        (<div className={styles.CheckouModalComponent}>
            ...
            <div className={styles.FormArea}>
                ...
                <div className={styles.TotalTransaction} id={styles[`${transactionPossible}`]}>
                    <p className={styles.TotalValue}>Valor Total</p>
                    {transactionPossible === 'ok' ?
                        <p>R$ {Number((sellQty + buyQty) * shareSelected.price).toFixed(2).replace('.', ',')}</p> :
                        <p><FontAwesomeIcon icon={faExclamation} size='2xl'/></p>
                    }
                </div>
